...
if (Get-Command g++ -ErrorAction SilentlyContinue) {
    ...
    Write-Host "âœ“ Found g++" -ForegroundColor Green
} elseif (Get-Command clang++ -ErrorAction SilentlyContinue) {
    $compiler = "clang++"
    $compilerArgs = @("-std=c++17", "-Wall", "-O2")
    Write-Host "âœ“ Found clang++" -ForegroundColor Green
} elseif (Get-Command cl -ErrorAction SilentlyContinue) {
    $compiler = "cl"
    $compilerArgs = @("/std:c++17", "/EHsc", "/O2", "/W3")
    Write-Host "âœ“ Found MSVC (cl)" -ForegroundColor Green
} else {
    Write-Host "âœ— No C++ compiler found in PATH" -ForegroundColor Red
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  âœ“ Built: $testOutput" -ForegroundColor Green
    } else {
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  âœ“ Built: $fuzzOutput" -ForegroundColor Green
    } else {
        Write-Host "  âœ— Build failed with exit code $LASTEXITCODE" -ForegroundColor Red
    }
} catch {
...
if (Test-Path "$outDir\test_formula_parser.exe") {
    Write-Host "âœ“ test_formula_parser.exe ready" -ForegroundColor Green
    Write-Host "  Run: .\build\test_formula_parser.exe" -ForegroundColor Gray
} else {
    Write-Host "âœ— test_formula_parser.exe not built" -ForegroundColor Red
}

if (Test-Path "$outDir\formula_fuzz_tester.exe") {
    Write-Host "âœ“ formula_fuzz_tester.exe ready" -ForegroundColor Green
    Write-Host "  Run: .\build\formula_fuzz_tester.exe [--iterations N]" -ForegroundColor Gray
} else {
    Write-Host "âœ— formula_fuzz_tester.exe not built" -ForegroundColor Red
}
