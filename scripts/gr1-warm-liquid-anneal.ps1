...
$Output = Join-Path $Root "$OutputRoot/GR-1C-warm-liquid-anneal"
& $Exe --input (Join-Path $Root "examples/golden_runs/GR-1C-warm-liquid-anneal.gr1") --output $Output
python (Join-Path $Root "tools/render_gr1_report.py") $Output
Write-Host "GR-1C warm-liquid-anneal workbook and PNGs: $Output/post_render" -ForegroundColor Green
