#!/usr/bin/env python3
"""
gen_empirical_ref.py
Writes the full empirical_reference.hpp from Python so encoding issues
with the VS extension file tool are bypassed.
"""
import os

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT  = os.path.join(ROOT, "atomistic", "core", "empirical_reference.hpp")
...
with open(OUT, 'w', newline='\n') as f:
    f.write(CONTENT)
print(f"Written {len(CONTENT)} bytes -> {OUT}")
