...
$Output = Join-Path $Root "$OutputRoot/GR-1A-cryogenic-quench"
& $Exe --input (Join-Path $Root "examples/golden_runs/GR-1A-cryogenic-quench.gr1") --output $Output
python (Join-Path $Root "tools/render_gr1_report.py") $Output
Write-Host "GR-1A cryogenic-quench workbook and PNGs: $Output/post_render" -ForegroundColor Green
