...
\fancyhead[R]{\small\textit{Condensed Reference (v5.14.1)}}
\fancyfoot[C]{\thepage}
...
{\normalsize Version 5.14.1 --- Day 88 Continuity Revision}\\[0.3em]
...
Modern computational materials science has a gap between fully resolved molecular dynamics and continuum descriptions. Quantum methods are too expensive at intermediate scales, while continuum models assume constitutive behaviour rather than generating it. VSEPR-SIM treats the Formation Engine, VSIM orchestration, artifact pipeline, and near-meso bead layer as one reproducible instrument for generating and interrogating physically admissible states under explicit assumptions.
...
\item \textbf{Traceable authority.} The periodic table anchors particle identity. Reference molecular or materials records may extend identity only when source, method, confidence, and transformation lineage are explicit; model-derived records never become experimental truth silently.
\end{enumerate}
...
Extension points (quantum corrections, reactive potentials, ML surrogates, hosted control, and open-system bead exchange) remain capability-gated. Their presence in a schema or renderer does not establish scientific validation.
...
\subsection{VSIM, Artifacts, and Presentation}

VSIM is the reproducible orchestration surface. A script declares identity, state evolution, analysis, and export intent; execution produces scientific truth artifacts and a manifest linking source hash, seed, engine version, and generated files. Renderer and report products are sidecars: they may consume positions, states, events, and metrics but do not own or mutate scientific state.

The renderer-neutral contract permits independent OpenGL, BGFX, native CPU/GDI, R, XLSX, and hosted HTML consumers. A successful renderer launch proves presentation availability only. Scientific evidence requires the upstream artifact, schema, provenance, and validation result.
...
Same switching function applied.  \textbf{Known limitation:} Coulomb--integrator coupling produces energy drift in MD for ionic systems; use FIRE minimization only for charged systems in v3.0.0.
...
Near-meso bead & $10^3$--$10^6$ packets & Resolved packet state + exchange ledger \\
Continuum handoff & model dependent & Conserved boundary flux summaries \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Resolved Bead Packets and Open Exchange}

A bead is not merely a display sphere. It is a classical near-meso control volume containing a resolved example packet: constituent identity summary, represented mass, momentum, energy, local observables, internal memory state, and provenance. Mass, momentum, energy, and selected species may enter or leave through explicit flux events. For bead $b$ over $\Delta t$,
\begin{equation}
\Delta M_b = \Delta t\sum_f \dot{m}_{b,f}, \qquad
\Delta E_b = \Delta t\sum_f \dot{E}_{b,f} + W_b + Q_b,
\end{equation}
with equivalent vector accounting for momentum. Every exchange is recorded in a conservation ledger; no visual interpolation may create or destroy represented mass. Closing the system reduces net boundary exchange to zero, while an open system requires declared sources, sinks, and residual tolerances.
...
Validation is evidence-scoped. Unit, integration, stress, artifact, renderer, and scientific-reference checks each require criteria defined \emph{before} execution. Test discovery is inventory only; a build does not imply runtime, interactive, or scientific correctness.
...
\textbf{Evidence status:} validated regimes, model-derived regimes, exploratory regimes, and rejected regimes are reported separately. No blanket production certification is inferred from aggregate pass counts.
...
\begin{enumerate}
\item \textbf{Visual and runtime stress above 1,000 particles} --- shared-artifact parity across OpenGL, BGFX, and native CPU/GDI consumers
...
\item \textbf{Resolved bead exchange} --- mass, momentum, energy, and species flux ledgers for open near-meso control volumes
...
License: MIT \quad|\quad active development \quad|\quad evidence-scoped validation
\end{center}
