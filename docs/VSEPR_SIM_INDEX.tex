...
\midrule
LaTeX source files     & 45 (43 sections + 2 master) \\
Compiled PDFs          & 44 \\
...
\item \textbf{section\_sm\_identity\_layer} --- SM-channel identity layer, channel gates, $\kappa$ scores
\item \textbf{section\_ikk\_kernel\_bridge} --- IKK theory $\to$ VSIM kernel mapping: scale ladder, hidden-active, fuzz proxy
...
\begin{enumerate}
\item \textbf{section\_sm\_identity\_layer} --- SM-channel interaction model: conservation gates, $\kappa_{ij}^{\mathrm{total}}$, $\Delta I$, annihilation
\item \textbf{section\_ikk\_kernel\_bridge} --- IKK base theory to VSIM: scale ladder, $H_j = \mathbf{c}_j^\top \mathbf{W}\mathbf{c}_j$, hidden-active, Doc III Kernel V4
