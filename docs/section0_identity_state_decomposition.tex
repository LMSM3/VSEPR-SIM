...
\begin{remark}[Realisation: Fuzzy Ball Model]
\label{rem:fuzzy-ball}
The Container Geometry Approximation~$\Omega_i$ is realised in the
coarse-grained visualisation subsystem as the \emph{Fuzzy Ball Model}
(see \texttt{docs/section\_fuzzy\_ball\_model.tex}).  The
correspondence is:
%
\begin{center}
...
\begin{tabularx}{\linewidth}{l c >{\raggedright\arraybackslash}X}
  ...
  $\Omega_i$ component
    & $\longleftrightarrow$
    & Fuzzy Ball implementation \\
  \midrule
  $R_i$ (effective radius)
    & & \texttt{effective\_radius}: probe-radius or mass-derived,
        shell extent of Layer~1 \\
  $\mathcal{T}_i(\theta,\phi)$ (angular texture)
    & & $S^{(k)}(\theta,\varphi)$: three-channel SH descriptor
        field, rendered as Layer~2 heatmap \\
  $\rho_i(r)$ (radial density)
    & & Radial exaggeration parameter~$\xi$: deforms shell
        proportionally to local field strength \\
  $\Sigma_i$ (structural role)
    & & \texttt{structural\_role} field on \texttt{Bead} and
        \texttt{BeadVisualRecord}; biases per-channel kernel
        weights \\
  $\Lambda_i$ (stability class)
    & & \texttt{stability\_class} field; inspectable in the
        bead inspector panel \\
  \bottomrule
\end{tabularx}
\end{center}
%
The angular texture undergoes a three-channel generalisation:
$\mathcal{T}_i \to \{S^{(0)},\, S^{(1)},\, S^{(2)}\}$, with a
synthesised contraction recovering the single-scalar §0 form.
Implementation: \texttt{coarse\_grain/vis/bead\_visual\_record.hpp},
\texttt{coarse\_grain/core/bead.hpp}.
\end{remark}
...
% ════════════════════════════════════════════════════════════════════════════
%  0.3.7 — Nano–Atomistic Boundary Extensions
% ════════════════════════════════════════════════════════════════════════════

\bigskip
\subsubsection*{0.3.7\quad Nano–Atomistic Boundary Extensions}
\addcontentsline{toc}{subsubsection}{0.3.7 Nano–Atomistic Boundary Extensions}
\label{sec:nano-atomistic}
...
At the nano–atomistic boundary, emergent structure is highly sensitive to
...
\end{equation}
where $M(t^{*})$ is an atomistic domain descriptor at the end of formation
(or at a chosen observation epoch~$t^{*}$).

This is framed positively because it is exactly what makes the
nano–atomistic boundary scientifically valuable: formation is not noise,
it is structure selection.

% ·········································································
\paragraph{0.3.7.3\quad Lower Atomistic Domain Descriptor (Precise Boundary
  Definition).}
\label{sec:lower-atomistic}
\mbox{}\\[0.4em]

Define the ``lower atomistic'' boundary as the regime where:
\begin{itemize}
...
the formal bridge between formation physics and atomistic realism.

% ════════════════════════════════════════════════════════════════════════════
\vfill
\begin{center}
  \rule{0.4\textwidth}{0.4pt}\\[0.5em]
  \textit{Formation Engine Methodology v3.0.0 —
