...
// Build an XYZMolecule from the internal Molecule representation.
// Atoms with invalid (NaN/Inf) coordinates are included so that the caller
// can choose to filter them via IOOptions::skip_invalid_atoms.
static vsepr::io::XYZMolecule mol_to_xyz_molecule(
    const Molecule& mol,
    const PeriodicTable& ptable,
    const std::string& comment)
{
    vsepr::io::XYZMolecule xyz;
    xyz.comment = comment;
    for (size_t i = 0; i < mol.num_atoms(); ++i) {
        const Element* elem = ptable.by_Z(mol.atoms[i].Z);
        if (!elem) continue;
        xyz.atoms.emplace_back(elem->symbol,
                               mol.coords[3*i],
                               mol.coords[3*i + 1],
                               mol.coords[3*i + 2]);
    }
...
int BuildCommand::Execute(const std::vector<std::string>& args) {
    ...
    try {
        ...
        if (discover_mode) {
            ...
            if (enable_thermal) {
                Display::Step("Initializing Thermal Module V3.0.0...");
        ...
        if (!output_file.empty()) {
            ...
            vsepr::io::XYZMolecule xyz_mol = mol_to_xyz_molecule(
                mol, ptable, formula + " - generated by VSEPR-Sim");

            vsepr::io::IOOptions opts;
            opts.skip_invalid_atoms = true;
            opts.precision = 6;

            const auto status = vsepr::io::save_structure(output_file, xyz_mol, opts);
            if (status.is_error()) {
                Display::Error("Cannot write to file: " + output_file);
                Display::Info(status.error().to_string());
                return 1;
            }
        ...
        if (watch) {
            ...
            {
                vsepr::io::XYZMolecule xyz_mol = mol_to_xyz_molecule(
                    mol, ptable, formula + " - built with VSEPR-Sim");

                vsepr::io::IOOptions opts;
                opts.skip_invalid_atoms = true;
                opts.precision = 6;

                const auto status = vsepr::io::save_structure(temp_file, xyz_mol, opts);
                if (status.is_error()) {
                    Display::Error("Cannot launch visualization - failed to write temp file");
