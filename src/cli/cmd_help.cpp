...
int HelpCommand::Execute(const std::vector<std::string>& /*args*/) {
    ...
    std::cout << R"(
  VSEPR-SIM is a research-oriented atomistic simulation and analysis platform
  for deterministic molecular structure generation, interpretation, and
  scientific reporting. All predictions emerge from explicit classical mechanics:

  • Bond stretching (harmonic potential)
  • Angle bending and VSEPR domain repulsion
  • Torsional barriers and Van der Waals interactions
  • Thermal property analysis (Debye model, heat capacity)
  • Crystal lattice generation and wind-particle dynamics
  • SolidWorks STEP import and batch thermal pipelines
  • Coarse-grained multi-scale modeling
  ...
  relax molecular structures to their equilibrium configuration.
    ...
    std::cout << "  build          Build molecules from chemical formulas\n"
              << "  viz            Interactive atomistic visualization\n"
              << "  therm          Analyze thermal properties and bonding\n"
              << "  cg             Coarse-grained console (scene, inspect, env, interact)\n"
    ...
    std::cout << "  \033[32m▶\033[0m  \033[1mvsepr build random --watch\033[0m  \033[32m← Try this first!\033[0m\n"
              ...
              << "  vsepr --viz --preset shell --beads 12\n"
              << "\n"
    ...
    std::cout << "  Full documentation available at: docs/\n"
              ...
              << "    • CLI_WALKTHROUGH.txt - Step-by-step CLI walkthrough\n"
              << "    • FILE_FORMATS.md     - XYZ/XYZA/XYZC/XYZF specification\n"
              << "    • VALIDATION_REPORT.md - Validation campaign results\n";
    Display::BlankLine();
