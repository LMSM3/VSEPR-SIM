...
Renderer::Renderer()
    : background_{0.05f, 0.15f, 0.08f}  // Dark green GPU theme
    , atom_scale_(0.5f)  // Smaller atoms by default
    , bond_radius_(0.15f)
    , show_bonds_(true)
    , show_box_(false)
    , sphere_vao_(0)
...
void Renderer::render(const FrameSnapshot& frame, int width, int height) {
    ...
    // Render periodic box if enabled (check frame metadata for box info)
    if (show_box_ && frame.stats.count("pbc_enabled") && frame.stats.at("pbc_enabled") > 0.5) {
        BoxOrtho box(
            frame.stats.count("box_x") ? frame.stats.at("box_x") : 10.0,
...
void Renderer::render_atoms(const FrameSnapshot& frame) {
    ...
    // Dynamic atom scaling: reduce size by 40% for interesting structures (4+ atoms)
    float dynamic_scale = atom_scale_;
    if (frame.positions.size() >= 4) {
        dynamic_scale *= 0.6f;  // 40% reduction for complex molecules
    }
    ...
    for (size_t i = 0; i < frame.positions.size(); ++i) {
        ...
        float radius = get_atom_radius(Z) * dynamic_scale;
...
void Renderer::render_box(const BoxOrtho& box) {
    // Render periodic box wireframe as 12 lines forming a rectangular box
    // Box is centered at origin with dimensions box.L
    
    float view[16], projection[16];
    camera_.get_view_matrix(view);
    camera_.get_projection_matrix(projection, 1280.0f / 720.0f);
    
    // Disable depth test for box edges (always visible)
    ...
    float Lx = static_cast<float>(box.L.x);
    float Ly = static_cast<float>(box.L.y);
    float Lz = static_cast<float>(box.L.z);
    
    // 8 corners of the box (origin at [0,0,0], extends to [Lx,Ly,Lz])
    float corners[8][3] = {
    ...
    for (int i = 0; i < 12; ++i) {
        int v0 = edges[i][0];
        int v1 = edges[i][1];
        glVertex3f(corners[v0][0], corners[v0][1], corners[v0][2]);
        glVertex3f(corners[v1][0], corners[v1][1], corners[v1][2]);
    }
    glEnd();
