...
# Pick up the same theme/rc as build_wsl.sh, if it has been injected already.
export VSEPR_REPO_ROOT="${VSEPR_REPO_ROOT:-$REPO_ROOT}"
[ -f "${REPO_ROOT}/deploy/vseprrc" ] && source "${REPO_ROOT}/deploy/vseprrc" || true
...
# CMakeLists.txt enforces the Ninja generator; fail fast with a clear message
# rather than letting CMake's own FATAL_ERROR surface deep in the log.
if ! command -v ninja &>/dev/null; then
    echo "[ERROR] ninja not found. Install with:"
    echo "        sudo dnf install -y ninja-build   (AlmaLinux/RHEL)"
    echo "        sudo apt install -y ninja-build    (Ubuntu/Debian)"
    exit 1
fi
