...
// Include the integrated chemistry database
...
void test(const string& name, bool condition, const string& msg = "") {
...
void test_coverage_audit() {
    ...
    for (uint8_t Z = 1; Z <= 118; ++Z) {
        ...
        string symbol = chem_db.get_symbol(Z);
        
        // Check Z matches
        if (chem.Z != Z && chem.Z != 0) {
            cout << "  [Z=" << (int)Z << " " << symbol << "] Z mismatch: " << (int)chem.Z << "\n";
            missing_count++;
            continue;
        }
        ...
        if (chem.lj_sigma <= 0.0 || !std::isfinite(chem.lj_sigma)) {
            cout << "  [Z=" << (int)Z << " " << symbol << "] Bad LJ sigma: " 
                      << chem.lj_sigma << "\n";
            bad_lj_count++;
        }
        if (chem.lj_epsilon < 0.0 || !std::isfinite(chem.lj_epsilon)) {
            cout << "  [Z=" << (int)Z << " " << symbol << "] Bad LJ epsilon: " 
                      << chem.lj_epsilon << "\n";
            bad_lj_count++;
        }
        ...
        if (chem.manifold != vsepr::BondingManifold::NOBLE_GAS && 
            ...
            if (chem.allowed_valences.empty()) {
                cout << "  [Z=" << (int)Z << " " << symbol << "] No valence patterns (manifold=" 
                          << (int)chem.manifold << ")\n";
                no_valence_count++;
            }
        }
    ...
    auto check_element = [&](uint8_t Z, const string& expected_symbol, 
...
void test_ionic_molecules() {
    ...
    cout << "\n  Li+ patterns:\n";
    for (const auto& v : Li_valences) {
        cout << "    bonds=" << v.total_bonds << ", coord=" << v.coordination_number 
