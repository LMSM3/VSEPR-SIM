...
// Produces one .demo.dynx + .demo.X per record that has a dynx_path set.
// out_dir is the directory to write demo artifacts into.
// -----------------------------------------------------------------------------

struct DemoArtifactEntry {
    std::string case_id;
    std::string bundle_path;  // path to emitted .demo.X
    bool        ok = false;
    std::string error;
};

struct DemoArtifactResult {
    std::vector<DemoArtifactEntry> entries;
    int ok_count   = 0;
    int fail_count = 0;
};

// run_demo_artifacts() is called after all cases complete.
// records must carry:
//   records[i].run_id       — used as case_id
//   records[i].axis_values["dynx_path"]  — path to the run's .dynx archive
//   records[i].axis_values["vsim_path"]  — path to originating .vsim (optional)
